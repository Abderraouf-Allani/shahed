(function () {
  ...
  var state = {
    ...
    ayahQuery: '',
    surahQuery: '',
    memProfile: (function () { try { return localStorage.getItem(LS.memProfile) === 'child' ? 'child' : 'adult'; } catch (e) { return 'adult'; } })(),
    tagQuery: '',
    selectedTagId: null,
    suggestCatId: null,
    suggestSel: {},
  ...
  var MEM_ICON_HIDE = '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="5" y1="12" x2="19" y2="12"/></svg>';
  ...
  function renderMemorize() {
    ...
    html += '<label class="mem-label">نمط الحفظ</label>';
    html += '<div class="plan-seg mem-profile-seg">';
    html += '<label class="plan-seg-opt"><input type="radio" name="memProfile" value="adult"' + (state.memProfile === 'child' ? '' : ' checked') + '> بالغ</label>';
    html += '<label class="plan-seg-opt"><input type="radio" name="memProfile" value="child"' + (state.memProfile === 'child' ? ' checked' : '') + '> طفل</label>';
    html += '</div>';
    ...
    document.querySelectorAll('input[name="memProfile"]').forEach(function (r) {
      r.addEventListener('change', function () {
        var v = document.querySelector('input[name="memProfile"]:checked');
        state.memProfile = (v && v.value === 'child') ? 'child' : 'adult';
        try { localStorage.setItem(LS.memProfile, state.memProfile); } catch (e) {}
      });
    });
