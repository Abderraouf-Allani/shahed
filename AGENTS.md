...
Memorize-page recitation controls follow the **active riwaya** (same as reader): the learner hears each ayah of the session and can loop each ayah N times (or forever) and/or loop the whole memorize session range N times (or forever). Sources: qaloon → quranpedia recitation 257 (`https://files.quranpedia.net/recitations/257/{surah%03d}{ayah%03d}.mp3`; verified `200 audio/mpeg`, `access-control-allow-origin: *`, supports Range); hafs → the same everyayah Husary files as the reader. Shared builder `riwayaAudioUrl(surah, ayah)` in `app.js` resolves by `currentRiwaya()`; both `memAudioUrl` and `rdrAudioUrl` delegate to it.
...
Reader-page recitation follows the active riwaya (`currentRiwaya()`):
- Hafs → `https://everyayah.com/data/Husary_128kbps/{surah%03d}{ayah%03d}.mp3`.
- Qaloon → the same quranpedia 257 files as memorize (see `riwayaAudioUrl`).
- **Numbering == the active riwaya's data set**: `data/quran.json` (qaloon) is no-basmala (Baqara 285, total 6214); `data/hafs.json` is Tanzil with-basmala (Baqara 286, total 6236 — everyayah's native count). Audio ayah index = the rendered ayah number 1:1; never re-derive offsets. The memorize session uses the same active-dataset numbering (`riwayaAudioUrl`).
- Reader audio lives in `rdr*` functions (state `rdrAudio`, current surah `rdrSurah`), stopped on route change / riwaya toggle / re-render of the reader; highlight is `.verse.rdr-playing`. Memorize audio mirrors this: `memStopAudio()` runs at the top of `renderMemorize()` (route change, riwaya toggle, re-render) and in `render()` when leaving `#/memorize`; highlight is `.verse.mem-playing`.