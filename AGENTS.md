...
- Qaloon → the same quranpedia 257 files as memorize.
- **Numbering == the active riwaya's data set**: `data/quran.json` (qaloon) is no-basmala (Baqara 285, total 6214); `data/hafs.json` is Tanzil with-basmala (Baqara 286, total 6236 — everyayah's native count). Audio ayah index = the rendered ayah number 1:1; never re-derive offsets.
- Reader audio lives in `rdr*` functions (state `rdrAudio`, current surah `rdrSurah`), stopped on route change / riwaya toggle / re-render of the reader; highlight is `.verse.rdr-playing`.